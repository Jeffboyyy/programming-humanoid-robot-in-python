...
class PIDController(object):
    ...
    def control(self, target, sensor):
        ...
        '''
        # YOUR CODE HERE
        self.u = self.u + (self.Kp + (self.Ki*self.dt) + self.Kd/self.dt)*(target - ((sensor) + self.dt * self.u)) - ((self.Kp + 2*self.Kd/self.dt)*self.e1) + (self.Kd/self.dt*self.e2) #Equation from lesson
        self.e2 = self.e1   #updating stats
        self.e1 = target - ((sensor) + self.dt * self.u)   #updating stats
        self.y.append(sensor)   #appending to list as output Folie6
